...
class GitHubSource(FirmwareSource):
    ...
    def download(
        ...
        try:
            ...
            if not quiet:
                ...
                resolved_pattern = self._get_resolved_pattern(release.tag_name)
                print(f"🔍 Asset pattern: {resolved_pattern}")
    ...
    def _get_resolved_pattern(self, version: str) -> str:
        ...
        return self.asset_pattern.replace("${revision}", version)
    ...
    def _find_matching_asset(self, release):
        ...
        pattern_resolved = self._get_resolved_pattern(release.tag_name)
